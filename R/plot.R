#' Plot quantiles
#'
#' @param x an object of class \code{quantile}.
#' @param ... additional parameters (compatibility).
#'
#' @details Info
#' @export

plot.quantile <- function(x, ...) {
  if(nrow(x$quantiles) > 1){
  quantiles <- estimate <- lower_ci <- upper_ci <- empirical <- NULL
  colors <- c("est" = "black", "emp" = "red")
  ggplot(data.frame(x$quantiles),aes(quantiles,estimate))+ geom_line(aes(color = "est")) + geom_ribbon(aes(ymin=lower_ci,ymax = upper_ci),alpha =0.2) + theme_bw() + labs(x= "Probability",  y= "Quantile", color = "") + geom_line(aes(y=empirical, col='emp'))+ scale_color_manual(values = colors)
  }
  else{
...
plot.return_level <- function(x, ...) {
  if(nrow(x$quantiles) > 1){
    Level <- estimate <- lower_ci <- upper_ci <- empirical <- NULL
    colors <- c("est" = "black", "emp" = "red")
    ggplot(data.frame(x$quantiles),aes(x=Level,y=estimate))+ geom_line(aes(color = "est")) + geom_ribbon(aes(ymin=lower_ci,ymax = upper_ci),alpha =0.2) + theme_bw() + labs(x= "Levels",  y= "Return Levels", color = "") + geom_line(aes(y=empirical, col='emp'))+ scale_color_manual(values = colors)
  }
  else{
    ..density.. <- x.complete <-  NULL
...
plot.VaR <- function(x, ...) {
  if(nrow(x$quantiles) > 1){
    ...
    colors <- c("est" = "black", "emp" = "red")
    ggplot(data.frame(x$quantiles),aes(x=VaR_Level,y=estimate))+ geom_line(aes(color = "est")) + geom_ribbon(aes(ymin=lower_ci,ymax = upper_ci),alpha =0.2) + theme_bw() + labs(x= "Var_Levels",  y= "VaR", color = "") + geom_line(aes(y=empirical, col='emp'))+ scale_color_manual(values = colors)
  }
  else{
    ..density.. <- x.complete <-  NULL
...
plot.ES <- function(x, ...) {
  if(nrow(x$quantiles) > 1){
    ...
    colors <- c("est" = "black", "emp" = "red")
    ggplot(data.frame(x$quantiles),aes(x=ES_Level,y=estimate))+ geom_line(aes(color = "est")) + geom_ribbon(aes(ymin=lower_ci,ymax = upper_ci),alpha =0.2) + theme_bw() + labs(x= "ES_Levels",  y= "ES", color = "") + geom_line(aes(y=empirical, col='emp'))+ scale_color_manual(values = colors)
  }
  else{
    ..density.. <- x.complete <-  NULL
...
plot.TVaR <- function(x, ...) {
  if(nrow(x$quantiles) > 1){
    ...
    colors <- c("est" = "black", "emp" = "red")
    ggplot(data.frame(x$quantiles),aes(x=TVaR_Level,y=estimate))+ geom_line(aes(color = "est")) + geom_ribbon(aes(ymin=lower_ci,ymax = upper_ci),alpha =0.2) + theme_bw() + labs(x= "TVaR_Levels",  y= "ES", color = "") + geom_line(aes(y=empirical, col='emp'))+ scale_color_manual(values = colors)
  }
  else{
    ..density.. <- x.complete <-  NULL
